...
# $Id: scipy_plus.py 1062 2014-10-10 15:31:02Z vladan $
"""
...
def chisquare_2(f_obs_1, f_obs_2, yates=False):
    ...
    p = stats.chisqprob(chisq, len(f_obs_1)-1)

    return chisq, p

def ttest_ind_nodata(mean_1, std_1, n_1, mean_2, std_2, n_2):
    """
    Student's t-test between two independent samples. Unlike in ttest_ind(), the
    samples (data) are not given, instead the basic statstical quantities (mean, 
    standard deviation and number of measurements) are used to do the test. 
    Returns t-value and a two-tailed confidence level.

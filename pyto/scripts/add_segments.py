...
$Id: add_segments.py 73 2008-03-17 14:09:28Z vladan $
Author: Vladan Lucic 
"""
...
# reasignes ids after the files are added
relabel = False
...
for (single_file, single_ids) in zip(labels_file_names, ids):
    ...
    labels_file.read(file=single_file, byteOrder=labels_byte_order, dataType=labels_data_type, arrayOrder=labels_array_order, shape=labels_shape)
    labels = pyto.segmentation.Segment(data=labels_file.data, copy=False, ids=single_ids)

    # add current segments
    result.add(new=labels, shift=curr_shift, remove_overlap=remove_overlap, dtype=out_labels_data_type, relabel=relabel)
    if shift is None:
        curr_shift = None
    else:
        curr_shift += shift

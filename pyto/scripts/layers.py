...
to these scripts are expected to be read from tomo_info.py file. This file
is located in the directory above the current one. These parameters are set 
in this script in lines having the following form:
...
$Id: layers.py 1153 2015-05-26 08:56:45Z vladan $
Author: Vladan Lucic 
"""
...
if tomo_info is not None: image_file = tomo_info.image_file_name
...
def make_layers(segments):
    ...
    if boundary_id_2 is None:
        ...
        layers =  segments.makeLayersFrom(
            bound=boundary_id_1, mask=layers_mask_id, thick=layer_thickness, 
        ...
        layers, dist =  segments.makeLayersBetween(
            bound_1=boundary_id_1, bound_2=boundary_id_2, mask=layers_mask_id, 
            nLayers=num_layers, nExtraLayers=num_extra_layers, 
...
def format_header(nested_segment_ids):
    ...
    # files
    lines.extend(common.format_file_info(name=image_file, 
                                         description="Image"))

...
# $Id: cluster.py 1081 2014-11-11 14:07:39Z vladan $
"""
__version__ = "$Revision: 1081 $"
...
# format, nested list can be used where ids in a sublist are uderstood in the
# "or" sense, that is all boundaries listed in a sublist form effectivly a 
# single boundary. 
...
def read_boundaries(boundary_ids):
    ...
    if is_multi_boundaries():
        bound, multi_boundary_ids = read_multi_boundaries(boundary_ids=boundary_ids)
    else:
...
def read_single_boundaries(boundary_ids):
    ...
    bound = pyto.segmentation.Segment.read(file=boundary_file_name, ids=boundary_ids,
...
def read_multi_boundaries(boundary_ids):
    ...
    for (l_name, b_ids) in zip(boundary_file_name, boundary_ids):
        curr_bound = pyto.segmentation.Segment.read(file=l_name, ids=b_ids,
               clean=True, byteOrder=boundary_byte_order, dataType=boundary_data_type,
               arrayOrder=boundary_array_order, shape=boundary_shape)
